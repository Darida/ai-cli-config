...
- **Check for comments copy-pasted across sibling implementations:** if the same comment, longer than 1 line, appears verbatim or near-verbatim in 3 or more different files, AND those files call the same shared method, extend the same base type, or implement the same interface, flag it. The comment almost certainly belongs once, on that shared entry point (the interface method, the base class/function it's implementing or calling), where every reader already lands — not copy-pasted at each implementation. If no single shared point exists to move it to, the underlying problem is likely the class/method name itself: flag that a clearer name may make the repeated explanation unnecessary.
- **Check for long pointer-only comments:** flag a comment that spans 2 or more lines whose entire content is directing the reader elsewhere ("see X's own doc for why...", "see Y above for the mechanism..."). A comment whose only job is pointing somewhere else should be a single short line: a few words stating the reason, then the pointer (e.g. "Cycle-breaking — see inject.Lazy's own doc."), not a multi-line lead-up to the same pointer.
...
- **Check for interface comments listing implementers:** an interface's own doc comment should state its contract — what a caller can rely on when holding this type — not describe or name its concrete implementations. Flag a comment on an `interface { ... }` declaration that names specific implementing structs/types (e.g. "X is the real implementation; Y is the test double") or describes how those implementations differ. That's implementation detail belonging on the implementing types themselves, not on the interface defining the contract they satisfy.
